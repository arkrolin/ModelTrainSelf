...
WorkerType = Literal["claudecode", "llm", "mock"]
...
WORKER_ENV_KEYS: dict[WorkerType, tuple[str, ...]] = {
    ...
    "llm": (
        ...
        "LLM_BASE_URL",
        "LLM_API_KEY",
        # Optional: "openai" (default) or "anthropic" — selects the wire protocol,
        # so any OpenAI-compatible server works through this one driver.
        "LLM_API_FORMAT",
    ),
    "mock": (),
}
...
PROMPT_REQUIRED_TOKENS_BY_GROUP: dict[str, dict[str, tuple[str, ...]]] = {
    "mock": {
        "reason.md": ("{max_intents}",),
        "explore.md": ("{intent_id}",),
        "explore_conclude.md": ("{intent_id}",),
        "bootstrap.md": ("{origin}",),
        "bootstrap_conclude.md": ("{origin}",),
    }
}
...
class LocalConfig(BaseModel):
    ...
    completed_action: Literal["keep", "remove"] = "keep"
...
class RuntimeConfig(BaseModel):
    ...
    worker_healthcheck: Literal["startup_and_task", "startup_only", "disabled"] = "startup_only"
...
class WorkerConfig(BaseModel):
    ...
    def validate_env(self) -> "WorkerConfig":
        # Local execution does not require LLM env keys (host CLI config is reused).
        # But if a worker DOES set them they must be non-empty strings.
        required = WORKER_ENV_KEYS[self.type]
        for key in required:
            if key in self.env and not self.env[key]:
                raise ValueError(f"worker {self.name} env key {key} must not be empty")
        if self.type == "mock":
...
def validate_prompt_resources(prompt_group: str) -> None:
    prompts_dir = resources.files("mts.dispatcher.prompts")
