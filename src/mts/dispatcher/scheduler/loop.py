...
from mts.dispatcher.config import DispatchConfig, LocalConfig, WorkerConfig
from mts.dispatcher.models import ReasonCheckpoint, RunningTask
...
from mts.dispatcher.scheduler.worker_select import choose_worker
from mts.dispatcher.workers.registry import get_driver
from mts.dispatcher.tasks.bootstrap import run_bootstrap_task
from mts.dispatcher.tasks.explore import run_explore_task
from mts.dispatcher.tasks.reason import run_reason_task
from mts.server.models import Intent, ProjectDetail, ProjectSummary
...
class DispatcherLoop:
    def __init__(self, config_path: Path, *, project_id: str | None = None):
        ...
        # Cooperative stop: the WebUI's "stop search" button cancels the running
        ...
        try:
            ...
            while not self._stop_requested.is_set():
                try:
                    ...
                    # Wait on the event, not time.sleep: a stop request during the
                    # retry backoff should take effect now, not one interval later.
                    self._stop_requested.wait(self.config.runtime.interval)
                    continue
                if once:
                    break
                self._stop_requested.wait(self.config.runtime.interval)
            if self._stop_requested.is_set():
                LOG.info("dispatcher loop stopped by request")
        finally:
            self.close()
    ...
    def _run_local_binary_check(self) -> None:
        ...
        LOG.warning(
            ...
            "configured and usable directly (e.g. `claude -p ...` works) — MTS injects no API keys.",
    ...
    def _try_dispatch_project(self, summary: ProjectSummary) -> bool:
        ...
        if self._project_running_task_count(summary.id) >= self.config.runtime.max_project_workers:
            ...
            return False
        if summary.trial_count >= summary.budget_max_trials:
            self._log_changed(
                f"{skip_scope}:budget_exhausted",
                logging.INFO,
                "skip project=%s because trial budget exhausted trials=%s budget=%s",
                summary.id,
                summary.trial_count,
                summary.budget_max_trials,
            )
            return False
    ...
    def _dispatch_bootstrap(self, project: ProjectDetail, intent: Intent) -> bool:
        ...
        # Acquire with claim, not heartbeat. The bootstrap intent is created
        # unclaimed, and heartbeat is strict-refresh: it renews an existing
        # holder and answers 403 to everyone else, which is what lets a lapsed
        # lease kill its worker. claim is the acquire op: 200 when the intent is
        # free (or already ours), 409 when another worker holds it.
    ...
    def _dispatch_explore(self, project: ProjectDetail, export_yaml: str, intent: Intent) -> bool:
        ...
        # Acquire with claim, not heartbeat: this intent is unclaimed, and
        # heartbeat only renews an existing holder (403 otherwise).
        claim = self.client.claim_intent(project.project.id, intent.id, worker.name)
        if claim.status_code in (403, 409):
            level = logging.INFO if claim.status_code == 403 else logging.WARNING
            LOG.log(
