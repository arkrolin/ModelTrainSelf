...
- 如果你在本次任务中已经复查过模型（看过训练曲线、统计过参数分布、看过梯度或 update_ratio），把那些**已经测到**的结论写进 `description`：具体数值或层名、由此判断的瓶颈、以及训练规模是依据哪条证据定的。这是后续 agent 判断"该不该加训练量"的依据。还没测到的不要补，也不要为了补齐它去跑任何命令。
