...
class WorkerDriver(abc.ABC):
    ...
    def process_env(self, worker: WorkerConfig) -> dict[str, str]:
        """Extra env vars this driver's subprocess needs, on top of `worker.env`.

        Lets a driver adapt to the host it was launched on without every task site
        knowing about it. Values here do not override an explicit `worker.env` entry.
        """
        return {}
