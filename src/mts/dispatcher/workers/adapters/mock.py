...
from mts.dispatcher.config import WorkerConfig, resolve_mock_behavior
from mts.dispatcher.workers.base import DriverResult, SeedSessionDriver
from mts.dispatcher.workers.health import HealthResult

_SCRIPT = """
import json,random,sys,time

# Module-level counter for improving metrics
_call_count = getattr(sys.modules[__name__], '_mock_call_count', 0)
_call_count += 1
sys.modules[__name__]._mock_call_count = _call_count
...
# Helper to generate improving metrics for MTS
def _mock_metric(goal_metric, goal_direction):
    base = 0.5 if goal_direction == "maximize" else 1.0
    improvement = _call_count * 0.01
    if goal_direction == "maximize":
        return min(1.0, base + improvement)
    else:
        return max(0.0, base - improvement)

goal_metric = prompt.get("goal_metric", "val_loss")
goal_direction = prompt.get("goal_direction", "minimize")
...
if phase=="bootstrap":
    trial_id = f"mock_trial_{_call_count}"
    metrics = {goal_metric: _mock_metric(goal_metric, goal_direction)}
    if outcome=="complete":
        print(json.dumps({"accepted":True,"data":{"fact":{"description":"mock fact for bootstrap","metrics":metrics,"trial_id":trial_id},"complete_description":"mock bootstrap complete from fact"}}, ensure_ascii=False))
    elif outcome=="fact":
        print(json.dumps({"accepted":True,"data":{"fact":{"description":"mock fact-only bootstrap result","metrics":metrics,"trial_id":trial_id}}}, ensure_ascii=False))
    elif outcome=="rejected":
        print(json.dumps({"accepted":False,"reason":"mock_rejected"}, ensure_ascii=False))
    else:
        print(json.dumps({"accepted":True,"data":{"fact":{"description":"mock invalid payload","metrics":metrics}}}, ensure_ascii=False))
    raise SystemExit(0)

if phase=="bootstrap_conclude":
    trial_id = f"mock_trial_{_call_count}_conclude"
    metrics = {goal_metric: _mock_metric(goal_metric, goal_direction)}
    if outcome=="fact":
        print(json.dumps({"accepted":True,"data":{"fact":{"description":"mock fact for bootstrap_conclude","metrics":metrics,"trial_id":trial_id}}}, ensure_ascii=False))
    elif outcome=="rejected":
        print(json.dumps({"accepted":False,"reason":"mock_rejected"}, ensure_ascii=False))
    else:
        print(json.dumps({"accepted":True,"data":{"complete":{"description":"mock invalid payload"}}}, ensure_ascii=False))
...
if outcome=="fact":
    ...
    trial_id = f"mock_trial_{label}_{_call_count}"
    metrics = {goal_metric: _mock_metric(goal_metric, goal_direction)}
    print(json.dumps({"accepted":True,"data":{"description":f"mock fact for {label}","metrics":metrics,"trial_id":trial_id}} , ensure_ascii=False))
elif outcome=="rejected":
    print(json.dumps({"accepted":False,"reason":"mock_rejected"}, ensure_ascii=False))
else:
    print(json.dumps({"accepted":True,"data":{}}, ensure_ascii=False))
